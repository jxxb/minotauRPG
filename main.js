   ...
   // if (
      ...
      //    ) {
         //       hWalls.children.forEach((hWall) => {
            ...
            //gets and wields weapon for character
            function getWeapon() {
               weapon.size.sx = 100;
               weapon.size.sy = 100;
            ...
            //random number/location generation
...
function loopy(ms) {
   ...
   // (character === horizontal wall)
   hWalls.children.forEach((hWall) => {
      const colisionYDistance = hWall.size.sy / 2 + character.size.sy / 2;
      const colisionXDistance = hWall.size.sx / 2 + character.size.sx / 2;
      let colisionX = (hWall.pos.x + (hWall.size.sx / 2)) - (character.pos.x + (character.size.sx / 2)); // x colision area
   ...
   minows.children.forEach((mino) => {
      ...
      if (weapon.visible && Math.sqrt(dx * dx + dy * dy) < (mino.size.sx / 2 + weapon.size.sx / 2)) {
         ...
         //when mino dies, item is dropped
   ...
   function endgame(){
      ...
      gameOver = true;
       ...
       //triggers Game Over text when character dies
       if(character.dead){
          endgame();
          setTimeout(()=>{
